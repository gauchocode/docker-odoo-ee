...
# Instalar openupgradelib al iniciar el contenedor
echo "📦 Instalando openupgradelib..."
...
if [ ${#EXTRA_PATHS[@]} -gt 0 ]; then
    ...
    # Crear copia base del archivo original
    ...
    # Reemplazar en el archivo copiado
    ...
    # Si no hay cambios, usar el config original
    cp "$ORIGINAL_CONFIG" "$MODIFIED_CONFIG"
fi
...
if echo "$INSTALLED_MODULES" | grep -Fxq "$MAIN_MODULE"; then
    ...
    exec odoo --config="$MODIFIED_CONFIG" -u l10n_ar_afipws -d "$POSTGRES_DB" --db-filter="$POSTGRES_DB" --db_host="$HOST" --db_port=5432 --db_user="$POSTGRES_USER" --db_password="$POSTGRES_PASSWORD"
else
...
#exec odoo -d odoo3 --init=base --load-language=es_AR --db_host="$HOST" --db_port=5432 --db_user="$POSTGRES_USER" --db_password="$POSTGRES_PASSWORD"
